<div>
    <form wire:submit="submit" class="flex flex-col gap-4">
        ...
        <div>
            <x-text-input placeholder="Password" wire:model.live="form.password" type="password"/>
            @error('form.password')
            <div class="text-sm italic font-semibold text-red-400">{{ $message }}</div>
            @enderror
        </div>
        <div>
            <x-text-input placeholder="Confirm Password" wire:model.live="form.password_confirmation" type="password"/>
        </div>
        <div>
